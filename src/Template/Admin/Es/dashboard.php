    ...
    <div class="be-bc-fff be-p-150" id="app" v-cloak>
        ...
        if (!$this->configEs->enable) {
            ...
            <div class="be-fw-bold">
                ES搜索引擎未启用！
                <el-link class="be-ml-100" type="primary" href="<?php echo beAdminUrl('Search.Config.dashboard', ['configName' => 'Es']); ?>">修改</el-link>
            </div>
            <?php
        } else {
                ...
                foreach ($this->indexes as $index) {
                    ...
                    if ($index['exists']) {
                        ...
                        <el-form ref="<?php echo $index['name']; ?>FormRef" :model="<?php echo $index['name']; ?>FormData">
                            <el-form-item class="be-mt-50" prop="number_of_shards" label="分片数" :rules="[{required: true, message: '请输入分片数', trigger: 'change' }]">
                                <el-input-number
                                        :min="1"
                                        :step="1"
                                        placeholder="请输入分片数"
                            ...
                            <el-form-item class="be-mt-50" prop="number_of_replicas" label="副本数" :rules="[{required: true, message: '请输入副本数', trigger: 'change' }]">
                                <el-input-number
                                        :min="0"
                                        :step="1"
                                        placeholder="请输入副本数"
            ...
            methods: {
                createIndex: function (formRef, formData) {
                    ...
                    this.$refs[formRef].validate(function (valid) {
                        if (valid) {
                            _this.loading = true;
                            _this.$http.post("<?php echo beAdminUrl('Search.Es.createIndex'); ?>", {
                ...
                deleteIndex: function (indexname) {
                    let _this = this;
                    _this.$confirm("本操作将删除索引中的所有数据，确认要删除么？", "删除确认", {
                        confirmButtonText: "确定",
                        cancelButtonText: "取消",
                        type: "warning"
                    }).then(function(){
                        _this.loading = true;
                        _this.$http.post("<?php echo beAdminUrl('Search.Es.deleteIndex'); ?>", {
                            formData: {
                                name: indexname
                            }
                        }).then(function (response) {
